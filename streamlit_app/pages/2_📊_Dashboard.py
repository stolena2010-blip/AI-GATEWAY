...
st.set_page_config(page_title="📊 Dashboard — AI GATEWAY KITARON", page_icon="🌿", layout="wide")

st.markdown(BRAND_CSS, unsafe_allow_html=True)
sidebar_logo()
# Extra page-specific styles handled by brand.py BRAND_CSS

st.html(brand_header("DASHBOARD — AI GATEWAY KITARON"))
st.html(profile_tab_css())

# ═══════  PROFILE TABS  ═══════
_PROFILE_ICONS = {"quotes": "📄", "orders": "📦", "invoices": "🧾", "delivery": "📋", "complaints": "📣"}
_profiles = load_all_profiles()

# ═══════  PERIOD FILTER BAR (shared across all tabs)  ═══════
...
# ═══════════════════════════════════════════════════════════════
#  RENDER DASHBOARD — called once per profile tab
# ═══════════════════════════════════════════════════════════════

def _render_dashboard(profile_name: str, profile_display: str, tab_key: str):
    """Render full dashboard content for one profile (or combined)."""
    if profile_name != "__all__":
        st.html(profile_banner(profile_name))
    # ── Load data ──
    if profile_name == "__all__":
        pnames = [p["profile_name"] for p in _profiles]
        all_entries = load_all_profile_log_entries(profile_names=pnames)
    else:
    ...
    def _prev_period_entries(all_ents, pk):
    ...
    with tab_accuracy:
        with st.expander("⚖️ משקלות דיוק (weights)", expanded=False):
            ...
            for i, lv in enumerate(("full", "high", "medium", "low", "none")):
                with wcols[i]:
                    new_weights[lv] = st.number_input(labels_w[lv], value=weights[lv], min_value=0.0, max_value=1.0, step=0.1, key=f"w_{lv}_{tab_key}")
            if st.button("💾 שמור משקלות", key=f"save_w_{tab_key}"):
    ...
    with tab_efficiency:
        ...
        if items_per_mail_positive:
            ...
            try:
                ...
                for label_b, cnt in buckets.items():
                    bar = "█" * cnt
                    st.text(f"{label_b}: {bar} {cnt}")
                ...
                for idx in range(len(table_data)):
                    ...
                    if new_val != orig_val and entry_ids[idx]:
                        save_entry_field(entry_ids[idx], "human_verified", new_val,
                                         profile_name=profile_name if profile_name != "__all__" else "quotes")
        else:
            st.info("אין נתונים")
    ...
    with tab_export:
        ...
        st.markdown("ייצוא כל הנתונים לקובץ Excel עם 6 גיליונות: סיכום כללי, נתוני מיילים, סיכום יומי, לקוחות, שולחים, סטטוס תפעולי.")

        if st.button("📥 הורד אקסל", width='stretch', key=f"export_btn_{tab_key}"):
            try:
                wb = build_workbook_dashboard(
                    email_entries,
                    run_label=f"dashboard:{period_key}:{profile_display}",
                ...
                st.download_button(
                    ...
                    data=excel_bytes,
                    file_name=f"AIGW_{profile_display}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key=f"download_btn_{tab_key}",
                )
        ...
        _reset_key = f"confirm_reset_{tab_key}"
        if _reset_key not in st.session_state:
            st.session_state[_reset_key] = False
        if st.session_state[_reset_key]:
            reset_stats_btn = st.button("⚠️ לחץ שוב לאישור איפוס", type="primary", key=f"reset2_{tab_key}")
        else:
            reset_stats_btn = st.button("🗑️ אפס סטטיסטיקה", type="secondary", key=f"reset1_{tab_key}")
        if reset_stats_btn:
            if st.session_state[_reset_key]:
                import shutil
                _pn = profile_name if profile_name != "__all__" else "quotes"
                src = LOG_DIR / "data" / _pn / "automation_log.jsonl"
                if not src.exists():
                    src = LOG_DIR / "automation_log.jsonl"
                if src.exists():
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    backup = src.parent / f"automation_log_backup_{ts}.jsonl"
...
# ═══════════════════════════════════════════════════════════════
#  BUILD PROFILE TABS + "סה"כ" TAB
# ═══════════════════════════════════════════════════════════════
_tab_labels = [f'{_PROFILE_ICONS.get(p["profile_name"], "📄")} {p["display_name"]}' for p in _profiles]
_tab_labels.append("📊 סה\"כ")
_profile_tabs = st.tabs(_tab_labels)

for _i, _profile in enumerate(_profiles):
    with _profile_tabs[_i]:
        _render_dashboard(_profile["profile_name"], _profile["display_name"], f"p{_i}")

# "סה"כ" combined tab
with _profile_tabs[len(_profiles)]:
    _render_dashboard("__all__", "סה\"כ", "all")
